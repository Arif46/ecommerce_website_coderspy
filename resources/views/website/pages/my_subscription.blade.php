...
@section('content')
                    ...
                    <div class="main-content">
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="float-md-right">
                                        <ul class="breadcrumb">
                                            ...
                                            <li class="active"><a href="{{ url('my-subscription') }}">{{__('My subscription')}}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row card no-border mt-4">
                            <div class="col-md-12 table-responsive">
                                <table class="table table-striped text-dark table-sm table-hover table-responsive-md">
                                    <thead>
                                    <tr>
                                        <td>#</td>
                                    ...
                                    @forelse($subscriptions as $subscription)
                                                ...
                                                <tr>
                                                    <td>{{++$i}}</td>
                                                    <td>{{$subscription->id}}</td>
                                                    <td>{{$subscription->shipping_charge}}</td>
                                                    <td>{{$subscription->subscription_amount}}</td>
                                                    <td>{{$subscription->payment_method == 1 ?'Paypal':'Credit Card'}}</td>
                                                    <td>{{$subscription->subscription_date}}</td>
                                                    <td>
                                                        <button  style="float: left;" data-toggle="modal" data-target="#subscription_modal{{$subscription->id}}" class="btn btn-success btn-sm"><i class="fa fa-eye"></i></button>
                                                    <form method="POST" action="{{ route('subscription-store.destroy',$subscription->id) }}">
                                                        @csrf
                                                        @method("DELETE")
                                                        <button onclick="return confirm('Are you sure to delete ?')" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></button></td>
                                                    </form>
                                                </tr>
                                        @empty
                                        <tr>
       ...
       @foreach($subscriptions as $subs)
           ...
           $total = 0;
           ?>
       <div class="modal fade" id="subscription_modal{{$subs->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
           <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-zoom product-modal" role="document">
               <div class="modal-content position-relative">
                   <div class="modal-header">
                       <h5 class="modal-title strong-600 heading-5">{{__('Subscription Details')}}</h5>
                                ...
                                <div class="modal-body gry-bg px-3 pt-2">
                                    <div class="card mt-4">
                                        <div class="card-header py-2 px-3 heading-6 strong-600 clearfix">
                                            ...
                                            <div class="float-right">Subscription No :<td>{{$subs->id}}</td></div>
                                        </div>
