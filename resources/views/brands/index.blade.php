...
<div class="row">
    ...
    <div class="col-sm-3">
        <form action="{{ route('brandImport') }}" method="POST" name="importform" enctype="multipart/form-data">
            @csrf
            <input type="file" name="file" class="form-control" required>
    </div>
    <div class="col-sm-6">
        <button class="btn btn-success">Import Excel Sheet</button>
        <a class="float-right btn btn-primary" href="{{route('brandExport')}}">Export Brands</a>
    </div>
    </form>
</div>
            ...
            <tbody>
                @foreach($brands as $key => $brand)
                    <tr>
                        ...
                        <td>{{$brand->name}}</td>
                        <td><img loading="lazy"  class="img-md" src="{{ asset('public/'.$brand->logo) }}" alt="Logo"></td>
                        <td>{{$brand->brands_parent_company}}</td>
                        <td>{{$brand->country}}</td>
                        <td>
