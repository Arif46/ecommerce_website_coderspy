...
<div class="col-lg-12">
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title">{{__('Add Income Tax')}}</h3>
        </div>
        ...
        <form class="form-horizontal" action="{{ route('tax.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="panel-body">
                ...
                <div class="form-group start_div">
                    ...
                    <div class="col-sm-4">
                          <label>{{__('Start Amount')}}</label>
                        <input type="number" placeholder="{{__('Start Amount')}}" id="start" name="start_amount" class="form-control {{ $errors->has('Start Amount') ? ' is-invalid' : '' }}" required>
                        @if ($errors->has('title'))
                            <span class="invalid-feedback" role="alert">
                                <strong style="color: red">{{ $errors->first('Start Amount') }}</strong>
                            </span>
                        @endif
                    </div>
